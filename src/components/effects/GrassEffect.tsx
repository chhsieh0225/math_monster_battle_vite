...
const GRASS_CORE: CoreConfig = {
  svgSize: 188,
  center: 94,
  radius: (fxLvl) => 24 + fxLvl * 4,
  gradientStops: [
    { offset: '0%', color: '#f0fdf4', opacity: 0.9 },
    { offset: '20%', color: '#4ade80', opacity: 0.78 },
    { offset: '48%', color: '#16a34a', opacity: 0.62 },
    { offset: '74%', color: '#14532d', opacity: 0.4 },
    { offset: '100%', color: '#020617', opacity: 0 },
  ],
  filter: {
    turbulenceType: 'fractalNoise',
    baseFreqValues: '0.84;0.46;0.2',
    displacement: {
      xChannel: 'R',
      yChannel: 'G',
    ...
    colorMatrix: '0.92 0 0 0 0 0 1.06 0 0 0 0.1 0.12 0.86 0 0 0 0 0 1 0',
  },
  outerFilter: (glow) => `drop-shadow(0 0 ${glow + 6}px #4ade80) drop-shadow(0 0 ${glow + 10}px #14532d)`,
};

const GRASS_RINGS: RingConfig = {
  count: (fxLvl) => Math.min(6, 2 + Math.floor(fxLvl / 2)),
  svgSize: [224, 116],
  center: [112, 58],
  baseRadius: (i) => [56 + i * 24, 17 + i * 5],
  strokeColors: ['rgba(74,222,128,0.78)', 'rgba(20,83,45,0.74)'],
  strokeWidth: (i) => 3.8 - i * 0.65,
  offset: ['30px', '18px'],
  delay: (D, i) => D + 0.08 + (i % 2) * 0.042 + Math.floor(i / 2) * 0.082,
  duration: (i) => 0.72 + i * 0.11,
  ringFilter: (glow) => `drop-shadow(0 0 ${glow}px rgba(74,222,128,0.65))`,
};

const GRASS_GLOW: GlowConfig = {
  gradient: (T, fxLvl) =>
    `radial-gradient(circle at calc(100% - ${T.right}) ${T.top}, rgba(74,222,128,${0.1 + fxLvl * 0.025}), rgba(22,101,52,${0.08 + fxLvl * 0.02}) 35%, rgba(2,6,23,${0.06 + fxLvl * 0.015}) 58%, transparent 74%)`,
  durMultiplier: 1.2,
};

const GRASS_ULTIMATE: UltimateConfig = {
  D: 0.3,
  core: GRASS_CORE,
  rings: GRASS_RINGS,
  glow: GRASS_GLOW,
...
export default function GrassEffect({ idx: moveIdx = 0, lvl = 1, target = DEFAULT_EFFECT_TARGET }: AttackElementEffectProps) {
  ...
  return (
    <UltimateEffect config={GRASS_ULTIMATE} ctx={grassEffectTemplate({ idx: moveIdx, lvl, target })}
      approach={
