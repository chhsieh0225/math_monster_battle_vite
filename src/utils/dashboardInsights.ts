...
const OP_GROUPS_BASE: GroupBase[] = [
  ...
  { id: 'mul', icon: '✖️', ops: ['×'] },
  { id: 'div', icon: '➗', ops: ['÷'] },
  { id: 'decimal', icon: '❄️', ops: ['dec_add', 'dec_frac', 'dec_mul', 'dec_div'] },
...
const GROUP_LABEL_KEYS: Record<string, string> = {
  ...
  decimal: 'dashboard.op.group.decimal',
...
const OP_NAME_FALLBACKS: Record<string, string> = {
  ...
  '×': 'Multiplication',
  '÷': 'Division',
  dec_add: 'Decimal Add/Sub',
  dec_frac: 'Decimal/Fraction Convert',
  dec_mul: 'Decimal Multiplication',
...
const OP_NAME_KEYS: Record<string, string> = {
  ...
  dec_add: 'dashboard.op.name.decAdd',
  dec_frac: 'dashboard.op.name.decFrac',
  dec_mul: 'dashboard.op.name.decMul',
  dec_div: 'dashboard.op.name.decDiv',
...
const WEAK_ACTIONS: Record<string, string> = {
  ...
  decimal: 'Practice decimal place alignment first, then decimal-fraction conversion and mul/div pacing.',
...
const WEAK_ACTION_KEYS: Record<string, string> = {
  ...
  decimal: 'dashboard.weak.action.decimal',
...
function createOpAgg(): OpAgg {
  return {
    ...
    '×': { a: 0, c: 0, ms: 0 },
    '÷': { a: 0, c: 0, ms: 0 },
    dec_add: { a: 0, c: 0, ms: 0 },
    dec_frac: { a: 0, c: 0, ms: 0 },
    dec_mul: { a: 0, c: 0, ms: 0 },
    dec_div: { a: 0, c: 0, ms: 0 },
    mixed2: { a: 0, c: 0, ms: 0 },
    mixed3: { a: 0, c: 0, ms: 0 },
    mixed4: { a: 0, c: 0, ms: 0 },
...
function createOpData(): OpData {
  return {
    ...
    '×': createOpDataEntry(),
    '÷': createOpDataEntry(),
    dec_add: createOpDataEntry(),
    dec_frac: createOpDataEntry(),
    dec_mul: createOpDataEntry(),
    dec_div: createOpDataEntry(),
